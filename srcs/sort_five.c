...
/*   Updated: 2021/10/15 16:48:11 by threiss          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void swap_five(t_list **stack_a, t_list **stack_b)
{
	...
	push_operat(stack_a, stack_b, "pb");
	if (data.lst_size_orig == 5)
		push_operat(stack_a, stack_b, "pb");
	fill_struct(&data, *stack_a, *stack_b);
	swap_three_cond(stack_a, data);
	while (!(is_sorted(*stack_a) == 1 && ft_lstsize(*stack_a) == data.lst_size_orig))
	{
		fill_struct(&data, *stack_a, *stack_b);
		swap_five_cond(stack_a, stack_b, &data);
		if (is_sorted(*stack_a) == 1 && ft_lstsize(*stack_a) == data.lst_size_orig)
			break;
	}
}
